...
class Timesheets(Document):
	...
	def calculate_total_amounts(self):
		...
		for d in self.get("time_logs"):
			...
			self.total_hours += flt(d.hours) - flt(d.destroyed_bundles)
	...
	def get_actual_timesheet_summary(self, operation_id):
		...
		return frappe.db.sql("""select
			...
			max(tsd.to_time) as to_time from `tabTimesheet Details` as tsd, `tabTimesheets` as ts where
	...
	def get_overlap_for(self, fieldname, args, value):
		...
		existing = frappe.db.sql("""select ts.name as name, tsd.from_time as from_time, tsd.to_time as to_time from
			`tabTimesheet Details` tsd, `tabTimesheets` ts where {0}=%(val)s and tsd.parent = ts.name and
			(
	...
	def get_last_working_slot(self, time_sheet, workstation):
		return frappe.db.sql(""" select max(from_time) as from_time, max(to_time) as to_time
			from `tabTimesheet Details` where workstation = %(workstation)s""",
...
def get_projectwise_timesheet_data(project, parent=None):
	...
	return frappe.db.sql("""select name, parent, billing_hours, billing_amount as billing_amt
			from `tabTimesheet Details` where parenttype = 'Timesheets' and docstatus=1 and project = %(project)s {0} and billable = 1
...
def get_timesheet(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql("""select distinct tsd.parent from `tabTimesheet Details` tsd,
...
def get_timesheet_data(name, project):
	...
	if project and project!='':
		...
		data = frappe.get_all('Timesheets',
...
def make_sales_invoice(source_name, item_code=None, customer=None):
	...
	timesheet = frappe.get_doc('Timesheets', source_name)
...
def set_time_sheet(target):
	if target.salary_slip_based_on_timesheet:
		target.set("timesheets", [])
		timesheets = frappe.db.sql(""" select * from `tabTimesheets` where employee = %(employee)s and start_date BETWEEN %(start_date)s AND %(end_date)s and (status = 'Submitted' or
			status = 'Billed')""", {'employee': target.employee, 'start_date': target.start_date, 'end_date': target.end_date}, as_dict=1)
...
def set_missing_values(time_sheet, target):
	doc = frappe.get_doc('Timesheets', time_sheet)
	...
	timesheets = frappe.db.sql(""" select * from `tabTimesheets` where employee = %(employee)s and start_date BETWEEN %(start_date)s AND %(end_date)s and (status = 'Submitted' or
			status = 'Billed')""", {'employee': doc.employee, 'start_date': doc.start_date, 'end_date': doc.end_date}, as_dict=1)
	for data in timesheets:
		target.append('piece_work', {
			'timesheets': data.name,
			'bundles': data.total_hours,
			'destroyed': data.destroyed_bundles
		})
	frappe.msgprint(_(target.total_working_hours))
...
def get_events(start, end, filters=None):
	...
	return frappe.db.sql("""select `tabTimesheet Details`.name as name,
			`tabTimesheet Details`.docstatus as status, `tabTimesheet Details`.parent as parent,
			from_time as start_date, hours, activity_type,
			`tabTimesheet Details`.project, to_time as end_date,
			CONCAT(`tabTimesheet Details`.parent, ' (', ROUND(hours,2),' hrs)') as title
		from `tabTimesheet Details`, `tabTimesheets`
		where `tabTimesheet Details`.parent = `tabTimesheets`.name
			and `tabTimesheets`.docstatus < 2
			and (from_time <= %(end)s and to_time >= %(start)s) {conditions} {match_cond}
		""".format(conditions=conditions, match_cond = get_match_cond('Timesheets')),
		{
			"start": start,
			"end": end
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	if customer:
		...
		return frappe. db.sql('''SELECT ts.name, tsd.activity_type, ts.status, ts.total_billable_hours, \
			tsd.sales_invoice, tsd.project  FROM `tabTimesheets` AS ts inner join `tabTimesheet Details` \
...
def get_list_context(context=None):
	return {
		...
		"row_template": "erpnext/templates/includes/timesheet/timesheet_row.html"
	}
