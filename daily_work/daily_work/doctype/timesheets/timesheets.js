...
frappe.ui.form.on("Timesheets", {
	...
	make_invoice: function(frm) {
		...
		dialog.set_primary_action(__("Make Sales Invoice"), () => {
			...
			return frappe.call({
				type: "GET",
				method: "daily_work.daily_work.doctype.timesheets.timesheets.make_sales_invoice",
				args: {
	...
	make_salary_slip: function(frm) {
		frappe.model.open_mapped_doc({
			method: "daily_work.daily_work.doctype.timesheets.timesheets.make_salary_slip",
			frm: frm
		});
	},
})
...
frappe.ui.form.on("Timesheet Details", {
	...
	activity_type: function(frm, cdt, cdn) {
		...
		frappe.call({
			method: "daily_work.daily_work.doctype.timesheets.timesheets.get_activity_cost",
			args: {
...
var calculate_time_and_amount = function(frm) {
	...
	for(var i=0; i<tl.length; i++) {
		if (tl[i].hours) {
            ...
            total_destroyed +=tl[i].destroyed_bundles;
			if(tl[i].billable){
	...
	frm.set_value("total_hours", total_working_hr - total_destroyed);
